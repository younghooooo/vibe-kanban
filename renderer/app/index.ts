...
// ===== TEMPLATE IMPORTS =====
import { getHeaderHTML } from "../widgets/header/index.js";
import { getSidebarHTML, getCategoryModalHTML } from "../widgets/sidebar/template.js";
import { getBoardViewHTML } from "../widgets/board/template.js";
import { getDetailViewHTML } from "../widgets/card-detail/template.js";
import { getAuthModalHTML } from "../features/auth/template.js";
import { getGhModalsHTML } from "../features/github-sync/template.js";
import { getLabelModalHTML } from "../entities/label/template.js";
import { getFindBarHTML, getGlobalSearchHTML } from "../features/search/template.js";
import { getRecentCardsHTML } from "../features/recent-cards/template.js";

// ===== MOUNT HTML TEMPLATES (DOM 쿼리 전에 실행) =====
(function mountTemplates() {
  const tc = document.getElementById("toastContainer");
  tc.insertAdjacentHTML("afterend", getHeaderHTML());
...
} from "../features/shortcuts/index.js";
import {
  currentLabelFilter,
...
import {
  ...
  closeRecentCards,
  initRecentCards,
} from "../features/recent-cards/index.js";
import {
  ...
  if (apiKeyInput) {
    apiKeyInput.addEventListener("change", async (e) => {
      ...
      if (v.startsWith("sk-ant-")) {
        ...
        if (r.ok) {
          toast(
            r.encrypted ? "API 키 저장 (암호화)" : "API 키 저장",
            "success",
          );
...
window.__mockPending = function (cardId) {
  ...
  if (!card) {
    console.warn("__mockPending: card not found", cardId || state.detailCardId);
    return;
  }
...
(async () => {
  try {
    ...
    console.error("initThemeAndSidebar failed", e);
  }
  try {
    initMoreMenu();
  } catch (e) {
    console.error("initMoreMenu failed", e);
  }
  try {
    initShortcuts();
  } catch (e) {
  ...
  try {
    ...
    console.error("initDetailView failed", e);
  }
  try {
    initCardSearch();
  } catch (e) {
    console.error("initCardSearch failed", e);
  }
  try {
    initGlobalSearch();
  } catch (e) {
    console.error("initGlobalSearch failed", e);
  }
  try {
    initRecentCards();
  } catch (e) {
    console.error("initRecentCards failed", e);
  }
  try {
    initBoardEvents();
  } catch (e) {
    console.error("initBoardEvents failed", e);
  }

  const savedModel = await window.api.getModel();
  const modelSelect = document.getElementById("modelSelect");
  const validModels = Array.from(modelSelect.options).map((o) => o.value);
  const resolvedModel = validModels.includes(savedModel)
    ? savedModel
    : "claude-sonnet-4-6";
